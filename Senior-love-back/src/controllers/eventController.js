...
export const eventController = {
  ...
  async addUserToEvent(req, res, next) {
    try {
      ...
      if (
        ...
        event.users.length >= event.availability
      ) {
      ...
      res
        .status(200)
        .json(event, {
          message: `${user.pseudo} inscrit à l'événement avec succès. Nombre d'inscrit = ${event.users.length}`,
        });
      console.log({
        message: `${user.pseudo} inscrit à l'événement avec succès. Nombre d'inscrit = ${event.users.length}`,
      });
  ...
  async create(req, res) {
    try {
      ...
      console.log(
        { message: `Événement "${event.name}" créé avec succès.` },
        event
      );
    } catch (error) {
      console.log(error);
  ...
  async update(req, res, next) {
    try {
      ...
      res.status(201).json(event);
      console.log(
        { message: `Événement "${event.name}" modifié avec succès.` },
        event
      );
    } catch (error) {
      console.log(error);
  ...
  async delete(req, res, next) {
    try {
      ...
      res
        .status(200)
        .json({ message: `Événement "${event.name}" supprimé avec succès.` });
    } catch (error) {
      console.log(error);
      res.status(500).json({ message: "Erreur Serveur" });
    }
